      ...
      rowGroup.selectAll(".cell")
        ...
        .on("mouseover", function(event, d) { 
          ...
          tooltip.transition()
            .duration(200)
            .style("opacity", .9);
          tooltip.html(`Click to see battles between ${sourceName} and ${targetName}`)
          ...
          if (battles.length > 0) {
            ...
            tooltip.html(`
              <div style="max-height: 300px; overflow-y: auto;">
                <h4 style="margin-top: 0;">Battles between ${nodes[d.y].name} and ${nodes[d.x].name}</h4>
                ${battleList}
              </div>
            `)
            .style("left", (event.pageX + 10) + "px")
            ...
            tooltip.transition()
              .duration(200)
              .style("opacity", .9);
            tooltip.html(`No recorded battles between ${nodes[d.y].name} and ${nodes[d.x].name}`)
              .style("left", (event.pageX + 10) + "px")
              .style("top", (event.pageY - 20) + "px");
          }
        });
